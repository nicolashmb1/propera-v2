...
| **# staff capture** post-finalize schedule | GAS (compare) | `extractScheduleHintStaffCapture` — `scheduleRaw` from `compileTurn` or `Preferred:` line; `appendPortalStaffScheduleNote` + optional `afterTenantScheduleApplied` | **V2 policy** | Deliberate divergence: **no** `TICKET_CREATED_ASK_SCHEDULE` block to staff when the finalize message has no parseable window; if window present, apply inline (events `STAFF_CAPTURE_INLINE_SCHEDULE` / `STAFF_CAPTURE_NO_SCHEDULE_PROMPT`). | **Low** | Legacy sessions stuck in SCHEDULE wait: short line instead of full `buildMaintenancePrompt("SCHEDULE")` for `#` (brain `core_staff_schedule_unparsed`). |
