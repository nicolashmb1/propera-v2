...
**Biggest intentional gaps vs full GAS:** full **`handleInboundRouter_`** graph (weak-issue / media-only branches, full `routeToCoreSafe_` parity), full **`handleLifecycleSignal_`** / sheet-style lifecycle, **Outgate template map**, **GAS-class brain lanes** (water, full amenity/leasing/vendor graphs), **canonical intake vision/CIG** queue, **Alexa/sensors**. **V2-native slices shipped separately** (not GAS parity): Communication Engine (**055**), Access Engine (**057**), portal Leasing Ops (**085**), vendor dispatch + inbound YES/NO (**069/073**), Conflict Mediation CME-2 (**067**), balance reminders (**098–101**).
...
| `21_AMENITY_ENGINE.gs` | Amenity lane | **GAS brain NOT STARTED** — **Access Engine V1** is a separate V2-native slice (**057**, `src/access/*`); see `ACCESS_ENGINE_BUILD_PLAN.md` |
| `22_WATER_ENGINE.gs` | Water lane | **GAS brain NOT STARTED** |
| `23_LEASING_ENGINE.gs` | Leasing lane | **GAS brain NOT STARTED** — **Portal Leasing Ops V1 shipped** (§12, **085**, `/api/portal/leasing/*`) |
| `24_COMM_ENGINE.gs` | Communications | **V2-native Communication Engine shipped** — not GAS `24` parity (**055**, `src/communication/`); see `COMMUNICATION_ENGINE.md` |
| `26_VENDOR_ENGINE.gs` | Vendor lane | **PARTIAL** — portal assign + dispatch + inbound YES/NO (**069**, **073**, `handleVendorInbound.js`); full GAS vendor graph not ported; see `VENDOR_LANE.md` |
...
| Emergency flag | `hardEmergency_` + `evaluateEmergencySignal_` — `18_MESSAGING_ENGINE.gs` (~373–603) | `inferEmergency` — `src/dal/ticketDefaults.js` (uses `hardEmergency_`, `detectEmergencyKind_`, `evaluateEmergencySignal_`) + intake-package safety propagation: `parseMaintenanceDraftAsync` passes `fastDraft.safety` from `compileTurn`; `coreMaintenanceFastPath` and `coreMaintenanceMultiTurn` prefer `fastDraft.safety.isEmergency` before falling back to `inferEmergency(issueText)` | **PORTED** | Does not include LLM `classify_` or `urgentSignals_` (non-emergency urgency) | **Low** | Sheet shape `emergency` Yes/No + `emergency_type`; drives `skipScheduling` in core. V2 supplement: non-English emergency text (e.g. Spanish "huele a gas") detected in `properaBuildIntakePackage` supplemental blocks flows to ticket row via `fastDraft.safety`. |
...
| Precursor ordering | `handleInboundRouter_` chain | `evaluateRouterPrecursor.js` + `computeCanEnterCore` staff guard — `routeInboundDecision.js` | **PARTIAL** | Explicitly does not invoke full router / core / Sheets | **Medium** | **Staff:** `#` → capture; **all other staff traffic** (including **empty body**) → `STAFF_LIFECYCLE_GATE` — never `PRECURSOR_EVALUATED` tenant intake. **`computeCanEnterCore`** also blocks `staffContext.isStaff` unless `STAFF_CAPTURE_HASH`. |
...
| Full router | `handleInboundRouter_` — `16_ROUTER_ENGINE.gs` | `runInboundPipeline.js` + `src/index.js` (Telegram + Twilio SMS/WA) | **PARTIAL** | Twilio **SMS** = compliance + `sms_opt_out`; **WhatsApp/Telegram** = no TCPA compliance writes. **Orchestrator order + core guards:** [ORCHESTRATOR_ROUTING.md](./ORCHESTRATOR_ROUTING.md). **Vendor:** identified vendor phones → `handleVendorInbound` (not stub); unidentified vendor/system traffic → lane stubs. Full GAS vendor/amenity/water **brain graphs** not ported | **Medium** | See `transportCompliance.js`, `routeInboundDecision.js` |
| Tenant Agent adapter (conversational front door) | GAS: N/A (new V2 product) | **`src/adapters/tenantAgent/`** + append + **find_related** (`findRelatedTenantTickets.js`, `handleTenantFindRelatedTicket.js`) | **PARTIAL** | **Shipped:** Sprints 1–3, Phases 4–6 (clarify, append, post-48h lookup). **Gaps:** append portal timeline/`changed_by_*`; production pilot sign-off; Phase 7 staff ETA. | **Medium** | **`TENANT_AGENT_ENABLED=0`** default. **`docs/TENANT_AGENT_ADAPTER.md`**. Migration **`063`**. |
...
| **Post–staff-capture unit / apt correction** | (product gap vs GAS SCAP) | After finalize the **`staff_capture_drafts`** row is deleted, so **`#dNN …`** cannot continue the draft. **V2:** staff (Telegram/SMS/WhatsApp/portal with staff) use **`tryPortalPmTicketMutation`**: strict **`Update <HUMAN_TICKET_ID> …`** (`parsePortalPmTicketRequest`) **or** deterministic **natural language** when strict parse misses (`src/dal/staffTicketAmendNl.js`) — verb / **`apt A to B`** / labeled fields + **human id anywhere** in the message, else **WI + issue/property/unit hints** via `resolveTargetWorkItemForStaff` (same resolver as staff lifecycle). Updates **`tickets.unit_label`** (+ **`work_items.unit_id`**) and other PM fields parsed from text. **No LLM** on this path. | **V2 policy** | Not a GAS line-for-line port; avoids reusing dead draft ids. | **Low** | Examples: `Update PENN-050426-6362 Apt 322.` · `change apt 322 to 323` (single open WI or add id) · `PENN-050426-6362 issue: …`. Do **not** prefix with `#` for ticket edits. |
...
5. **Other GAS brain lanes** — Water (`22`), full GAS amenity/leasing/vendor graphs (`21`, `23`, `26`): **not ported**. **V2-native equivalents (not GAS parity):** Communication Engine (`24` class — **055**), Access Engine (**057**), portal Leasing Ops (**085**), vendor dispatch + YES/NO (**069/073**), balance reminders (**098–101**). See §12 and `BRAIN_PORT_MAP.md`.  
...
| **DAL** | **SHIPPED** (+ vendor) | `src/dal/portalTicketAssignment.js` — `applyPortalTicketAssignment` (staff + **vendor** `assigned_type=VENDOR`, `work_items.owner_type`), `listStaffAssignableToProperty`, **`listVendorsForAssignment`**, `assertStaffAssignable`, **`assertVendorAssignable`**. |
| **Portal routes** | **SHIPPED** (+ vendor) | `POST /api/portal/tickets/:ticketId/assignment`; `GET /api/portal/properties/:code/staff-for-assignment`; **`GET /api/portal/vendors-for-assignment`**; **`PATCH /api/portal/program-lines/:id/vendor`**. |
...
| **propera-app cockpit** | **SHIPPED** (+ vendor) | `TicketDetailPanel`: staff/vendor tabs in reassignment modal; **`046`** vendors + preventive line vendor; `/api/pm/vendors-for-assignment`, `/api/program-lines/:id/vendor`. |
| **Audit** | **SHIPPED** | Every change appends `PORTAL_PM_TICKET_ASSIGNMENT` to `event_log` with actor, staff/vendor id, ticket key, trace id. |
