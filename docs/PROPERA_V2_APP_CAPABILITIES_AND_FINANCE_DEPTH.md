...
**Finance end state (owner intent, 2026-06):** For this portfolio, **Propera must become financially complete** — own payments, bank reconciliation, deposit lifecycle, automation-first capture, and a **full accountant report section** — so the owner can **retire Leasehold** after parallel-run proves parity. Leasehold ingest today is **transitional**; see [PROPERA_FINANCE_ROADMAP.md](./PROPERA_FINANCE_ROADMAP.md) (Owner cutover intent, Phase 5b, Phase 6 cutover gate) and [ACCOUNTING_SIGNAL_SCHEMA.md](./ACCOUNTING_SIGNAL_SCHEMA.md).
...
**Non-goals (today — phased goals for cutover):** Full GL, bank reconciliation, and accountant report packs are **not shipped yet** but are **required end state** for this portfolio (Phases 5b–6). Do not treat them as permanently out of scope. Yardi/AppFolio-class enterprise GL and corporate consolidated reporting remain out of scope unless explicitly scoped.
...
- **Property operating expenses (Phase 1d):** **`property_expenses`** table (**082**) — PM-entered tax, insurance, utilities, etc.; app expense tab + bill scan via V2 `expenseScanVision.js`; record payment via existing ledger POST.
- **Financial Intake V1 columns:** **`ticket_cost_entries.receipt_status`**, `voided_at`, `capture_idempotency_key` — migration **053** (chat/marker cost capture path).
...
### 2.6 Communications & rent reminders (V2-native, feature-flagged)

- **Communication Engine:** dedicated Twilio broadcast number, campaign draft/compose/send, audience preview, reply/delivery tracking — migrations **055**, **065**, **102**; `PROPERA_COMMUNICATION_ENGINE_ENABLED=1`; app `/communications/*` when `NEXT_PUBLIC_PROPERA_COMMUNICATIONS_ENABLED=1`. Spec: **COMMUNICATION_ENGINE.md**.
- **Balance reminder automation:** cron-driven rent reminders via the same comm engine + Leasehold snapshot balances — migrations **098–101**; `PROPERA_BALANCE_REMINDER_ENABLED=1`; portal settings API + app **`/settings/balance-reminders`**. Spec: **BALANCE_REMINDER_AUTOMATION.md**.

### 2.7 Optional field / lifecycle engines (flags)

- **Turnover engine:** `PROPERA_TURNOVER_ENGINE_ENABLED=1` — portal surface in app when wired (**039**).
- **Meter / utility batch reads:** meter run routes + storage (migrations **031**, **041** and `registerMeterRunRoutes`).
- **Access Engine (partial):** reservations, lifecycle jobs, staff + tenant portal surfaces — **057**, **066**; `PROPERA_ACCESS_ENGINE_ENABLED=1`. Spec: **ACCESS_ENGINE_BUILD_PLAN.md**.
- **Unit lifecycle:** occupancy history, unit assets, episode stamp — **087–090**; `PROPERA_UNIT_LIFECYCLE_ENABLED=1`. Spec: **UNIT_LIFECYCLE_BUILD_PLAN.md**.

### 2.8 Other shipped V2-native surfaces (not GAS brain parity)

| Surface | What ships | Flags / migrations |
|---------|------------|-------------------|
| **Portal Leasing Ops** | Expiring leases + prospect pipeline (CRM slice, not GAS leasing brain) | **085**; `PROPERA_LEASING_ENGINE_ENABLED` + app `NEXT_PUBLIC_PROPERA_LEASING_ENABLED` |
| **Vendor lane (partial)** | Portal assign + dispatch SMS; inbound YES/NO for identified vendor phones | **069**, **073**; `handleVendorInbound.js` — spec **VENDOR_LANE.md** |
| **Conflict Mediation (CME-2)** | Report violation + courtesy notice SMS | **067**, **071**; `PROPERA_CONFLICT_MEDIATION_ENABLED=1` |
| **Tenant portal** | Resident `/tenant/*` + V2 `/api/tenant/*` | **056**; tenant auth + push (**083**) |
| **Tenant Agent** | Conversational SMS/TG/WA front door → brain handoff | **063**; `TENANT_AGENT_*` flags |
| **Jarvis (staff voice / ask / plan)** | Read tools, proposal spine, portal Plan tab | **070**, **092**; `JARVIS_*` flags — **JARVIS_SPINE.md** |
| **Multi-org / Settings** | Org spine, channels, policies, onboarding | **074–078** — **MULTI_ORG_ARCHITECTURE.md** |
| **Open deck day chart** | Mobile tickets day curve (8a–8p) | JS-only (**061** comment migration); feature flags on V2 + app |
...
| Leasehold snapshot rent / balance / delinquency | **Yes (Phase 1.5)** — **`tenant_account_snapshots`** (**094–097**) when import runs; portfolio KPIs on **`/financial`**. | Native **`rent_postings`** (Phase 2) not started; snapshots are read-only incumbent truth. |
| Property operating expenses (parallel-run) | **Yes (Phase 1d)** — **`property_expenses`** (**082**); expense tab, bill scan, record payment. | Full AP / vendor ledger still Layer 2 gap. |
...
| Capability | Today | Gap |
|------------|-------|-----|
| Catalog **vendor** for assignment + dispatch | Yes (tickets + preventive lines) — **vendors** table + APIs; **vendor lane V0–V2** (assign, dispatch SMS, inbound YES/NO — **069**, **073**). | **Not** full vendor master (terms, tax IDs, insurance certs) or AP ledger unless you extend schema. |
...
| Income vs maintenance (NOI-style) | **Partial** — maintenance cost slice + **imported Leasehold snapshots** on **`/financial`** when **094–097** applied (billed, collected, balance due); **unit lease** (**049**) stores **expected** rent template but does not replace incumbent posting. | Native rent roll + owner packs (Phases 2–5) not built. |

---

## 6. Practical “where we stop” today

- **Stops at:** operational cost capture (incl. **053** receipt/void/idempotency), coarse categorization, optional **ticket-linked** tenant ledger posting, preventive costs, property/month rollups for maintenance spend; **owner-facing `/financial` portfolio** with **Leasehold snapshot KPIs** when imported (**094–097**); **property operating expenses** (**082**); **property-level units table** under finance; **per-unit lease snapshot** (**049**) and **per-unit ledger** (read posted + ticket charges, **manual POST** for one-offs); **automated rent reminders** via Communication Engine when configured (**098–101**).
- **Does not yet include:** vendor AP ledger, bank sync, **native Propera rent posting** (`rent_postings` — Phase 2), tenant payments/disputes as a product loop, budgets, owner PDF packs, or GL.

Use **Layer 1 + Phase 1.5 snapshot read** as the current product horizon; treat **Layers 2–5** as **sequenced product bets** (each usually needs schema + portal + app + policy, not a single “finance toggle”). Next finance schema migration: **103+** (see **PROPERA_FINANCE_ROADMAP.md**).
...
- Replacing QuickBooks/Yardi for **enterprise** consolidated GL in v1 — different buyer. “Ahead” here means **best-in-class property maintenance operations with economics on the same objects**, then **financial completeness for this portfolio** (payments, bank rec, reports, cutover from Leasehold) — sequenced in [PROPERA_FINANCE_ROADMAP.md](./PROPERA_FINANCE_ROADMAP.md).
...
| **P0** | **Phase 1.5 — incumbent snapshot read** | **Shipped (2026-06-08):** **094–097**, leasehold-bridge, `/financial` KPIs, office syncher spec. *Gap:* office PC syncher script + Task Scheduler not in repo yet. |
| **P0** | **Phase 1d — property operating expenses** | **Shipped:** **082** `property_expenses`, expense tab, bill scan, record payment. |
...
| **P2** | **Phase 2 — native rent roll / delinquency** | **Not started** — needs new migration **103+** (`rent_postings` or promoted snapshot). |
...
| **Defer** | Full **vendor AP**, **budget vs actual** — until Layer 1 + Phase 1.6 mimic are default workflow. | Phase 1.6 Step 2 **pilot** (WESTFIELD 101) shipped; expand building next. |
| **P0 (cutover)** | **Phase 1.6** mimic + **Phase 5b reports** + **Phase 6 bank rec** — owner path off Leasehold | Step 1 + Step 2 pilot shipped; next: validate 101 → full WESTFIELD → 5b reports → cutover |
...
*Last aligned to repo capabilities as of migrations **042–097**, **082** (property expenses), **053** (financial intake cost columns), **094–097** (Leasehold snapshots), **098–101** (balance reminders); V2 portal finance routes in `registerPortalRoutes.js`; and **propera-app** `/financial` + unit **lease/ledger** APIs + `/settings/balance-reminders`.*
