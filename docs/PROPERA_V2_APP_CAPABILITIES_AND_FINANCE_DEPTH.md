...
|---------|----------------------------|
| **`/financial`** | **Portfolio-first** owner view: property cards; **expected rent**, **tenant balances**, and **net owner cost (maintenance)** are live from Propera lease/ledger/maintenance reads. **Rent collected** / **delinquent** remain **placeholders** until a rent register or PMS snapshot is integrated. Sub-nav trimmed to **Portfolio** + **Maintenance spend**. |
| **`/financial/properties/[property]`** | **Units & balances** tab: sortable unit table with live **rent**, **selected-month posted payments**, **standing balance**, and **lease expiry** from the financial snapshot API; expandable rows show **last payment**, **security deposit**, **6-month payment history**, and **month-scoped vacancy loss** from canonical unit status history. **Maintenance** tab now includes a **12-month spend trend**, **current-month cost category breakdown**, **receipt preview/open affordances** per ticket row, and opens the shared **`TicketDetailPanel`** directly from the financial ticket list. |
