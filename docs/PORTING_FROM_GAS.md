...
| Unit label normalize (canonize / structured signal) | `normalizeUnit_` — `17_PROPERTY_SCHEDULE_ENGINE.gs` ~2247–2258 | `normalizeUnit_` — `src/brain/shared/extractUnitGas.js` (used by **`properaCanonizeStructuredSignal`**) |
...
| **compileTurn / intake package** | `compileTurn_`, `properaBuildIntakePackage_` — `08_INTAKE_RUNTIME.gs` + `07_PROPERA_INTAKE_PACKAGE.gs` | **Partial:** `src/brain/intake/compileTurn.js` (forwards **`propertiesList`**), `properaBuildIntakePackage.js`, `openaiStructuredSignal.js`, **`properaCanonizeStructuredSignal`** — `src/brain/intake/canonizeStructuredSignal.js` (GAS-style property grounding via **`resolvePropertyFromTextStrict`** / `phraseInNormalizedText` — `lifecycleExtract.js` when **`propertiesList`** non-empty). Enable **`INTAKE_COMPILE_TURN=1`**; optional LLM: **`OPENAI_API_KEY`** + **`INTAKE_LLM_ENABLED=1`** — [PARITY_LEDGER.md](./PARITY_LEDGER.md) §1–2 |
...
| **Orchestrator / route-to-core guards** | `handleInboundRouter_` / `routeToCoreSafe_` ordering — `16_ROUTER_ENGINE.gs`, `20_CORE_ORCHESTRATOR.gs` | **Partial:** `src/inbound/runInboundPipeline.js` + **`src/inbound/routeInboundDecision.js`** — lane, SMS compliance, core entry; **identified vendor** → `handleVendorInbound`; **unidentified** vendor/system → lane stubs; documented in **[ORCHESTRATOR_ROUTING.md](./ORCHESTRATOR_ROUTING.md)** |
...
| **Staff #capture tenant phone** | `findTenantCandidates_`, `enrichStaffCapTenantIdentity_` — `14_DIRECTORY_SESSION_DAL.gs`; name hints — `16_ROUTER_ENGINE.gs` | **`tenant_roster`** table (`012_tenant_roster.sql`); **`src/dal/tenantRoster.js`**; **`src/brain/gas/extractStaffTenantNameHintFromText.js`**; **`finalizeMaintenance.js`** (MANAGER uses resolved tenant phone only) |
