...
- **Flow (partial):** **`recomputeDraftExpected`** — pre-ticket slice (see ledger). **`intake_sessions`** + merge (includes **`issue_buf_json`** persistence + deterministic **attach classify** gates). **Unit:** GAS **`extractUnit_`** port — `extractUnitGas.js`. **`handleInboundCore`** (**thin entry**): **`emitMaintenanceCoreExit` / `emitMaintenanceCoreError`** (**`coreMaintenanceBoundaryLog.js`**); **`buildMaintenanceCoreDispatchContext`** (**`coreMaintenanceLoadContext.js`**) = **`CORE_ENTER`** → gates (**`handleInboundCoreGates.js`**) → staff vs tenant session (**`coreMaintenanceStaffCapture.js`**) → portal vs parse draft (**`coreMaintenancePortalDraft.js`**); then **`runCoreMaintenanceFastPath`** / **`runCoreMaintenanceMultiTurn`**; staff post-finalize receipt → **`finalizeReceiptStaffCaptureScheduleBranch`** (**`coreMaintenanceStaffFinalizeReceipt.js`**). **Deterministic issue clauses:** GAS **`parseIssueDeterministic_`** port — `src/brain/gas/issueParseDeterministic.js`, wired through **`properaBuildIntakePackage`** (fallback signal shape matches GAS `properaFallbackStructuredSignalFromDeterministicParse_`). Events: **`EXPECT_RECOMPUTED`**, **`TURN_SUMMARY`**, **`TICKET_CREATED_ASK_SCHEDULE`**, **`ATTACH_CLARIFY_REQUIRED`**, **`INTAKE_START_NEW`**, etc.
