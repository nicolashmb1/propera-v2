  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Compass-safe fallback message (no state mutation)
        if (fromKey) {
          sendRouterSms_(
            fromKey,
            renderTenantKey_("ERR_CRASH_FALLBACK", "en", {}),
            "CRASH_FALLBACK"
          );
        }
      } catch (_) {}
...
function renderTenantKey_(key, lang, vars) {
  ...
  // Prepend welcomeLine ONLY for allowlisted keys and ONLY if welcomeLine is non-empty.
  // (This keeps it deterministic: only those keys get it.)
  if (shouldPrependWelcome_(key)) {
    const wl = sanitizeTenantText_(String((V && V.__welcomeLine) || ""));
    if (wl) {
      // Avoid double-prepend if your template already contains the welcome text
  ...
  if (shouldAppendCompliance_(key)) {
...
function finishEmergencyIfReady_(sheet, dir, dirRow, row, lang) {
  ...
  if (hasProp && hasUnit && hasMsg) {
    ...
    if (tid) {
      reply_(renderTenantKey_("EMERGENCY_CONFIRMED_DISPATCHED_WITH_TID", lang, { ...baseVars, ticketId: tid }));
    } else {
      reply_(renderTenantKey_("EMERGENCY_CONFIRMED_DISPATCHED", lang, baseVars));
    }
    return true;
  }
  return false;
...
function looksActionableIssue_(s) {
  ...
  // Strong maintenance verbs / intents
  if (/(leak|leaking|drip|dripping|clog|backup|flood|water|toilet|sink|shower|tub|heater|ac|a\/c|heat|no heat|no hot|hot water|electric|outlet|breaker|sparks|smoke|gas|odor|mold|roach|bug|mouse|lock|door|key|window|broken|not working|doesn'?t work)/.test(t)) {
    return true;
  }

  // If it has a problem verb, treat as actionable
  if (/(not working|doesn'?t work|broken|won'?t|wont|stopped|leak|clog|smell|noise|sparks|tripping|overflow|backup|no heat|no hot|not heating|not cooling|doesn'?t drain|not draining)/.test(t)) return true;
...
function handleSmsSafe_(e) {
  ...
  try {
    ...
    // Soft fallback reply (template-driven)
    try {
      const baseVars = { brandName: BRAND.name, teamName: BRAND.team, __welcomeLine: welcomeLine };

      reply_(renderTenantKey_("ERR_GENERIC_TRY_AGAIN", "en", baseVars));
    } catch (_) {}
...
if (mode === "TENANT" && dirRow > 0 && domainDecision && domainDecision.detectedDomain === OPS_DOMAIN.CLEANING &&
  ...
  try {
    ...
    if (earlyResult && earlyResult.handled) {
      ...
      try { replyNoHeader_(renderTenantKey_("CLEANING_WORKITEM_ACK", lang, baseVars)); } catch (_) {}
      return;
    }
  } catch (_) {}
}
  ...
  if (!conversational[nextUp]) {
    ...
    if (effectiveStage === "PROPERTY") {
      ...
      if (cap && cap.hasUnit) {
        ...
        try { logInvariantFail_(phone, sidSafe || "", "STAGE_GUARD", "prev=" + prevUp + " next=" + nextUp + " replyKey=ASK_PROPERTY_GOT_UNIT"); } catch (_) {}
      ...
      if (cap && cap.hasIssue) {
        ...
        try { logInvariantFail_(phone, sidSafe || "", "STAGE_GUARD", "prev=" + prevUp + " next=" + nextUp + " replyKey=ASK_PROPERTY_GOT_ISSUE"); } catch (_) {}
    ...
    if (effectiveStage === "UNIT") {
      ...
      if (cap && cap.hasProp) {
        ...
        try { logInvariantFail_(phone, sidSafe || "", "STAGE_GUARD", "prev=" + prevUp + " next=" + nextUp + " replyKey=ASK_UNIT_GOT_PROPERTY"); } catch (_) {}
        reply_(renderTenantKey_("ASK_UNIT_GOT_PROPERTY", lang, v));
        return;
      }
      ...
      try { logInvariantFail_(phone, sidSafe || "", "STAGE_GUARD", "prev=" + prevUp + " next=" + nextUp + " replyKey=ASK_UNIT"); } catch (_) {}
    ...
    if (effectiveStage === "SCHEDULE") {
      ...
      try { logInvariantFail_(phone, sidSafe || "", "STAGE_GUARD", "prev=" + prevUp + " next=" + nextUp + " replyKey=ASK_WINDOW_SIMPLE"); } catch (_) {}
      replyNoHeader_(renderTenantKey_("ASK_WINDOW_SIMPLE", lang, baseVars));
      return;
    }
  }
}
...
try {
  if (mode === "TENANT") {
    ...
    if (ackOnly && !hasPending) {
      reply_(renderTenantKey_("TENANT_ACK_NO_PENDING", lang, baseVars));
...
try {
  ...
  if (modeNow === "TENANT" && dirRow > 0 && !stageNow2 && String(effectiveStage || "").toUpperCase() !== "SCHEDULE") {
    ...
    if ((isGreeting || isAck || !actionable) && !hasDraftData) {
      replyNoHeader_(renderTenantKey_("HELP_INTRO", lang, baseVars));
...
if (mode === "TENANT" && !createdByManager && tenantMatch && dirRow > 0 && !stageNow) {
  ...
  if (looksLikeGreetingOnly_(msgLower0) || looksLikeAckOnly_(msgLower0)) {
    replyNoHeader_(tenantMsg_("HELP_INTRO", lang, baseVars));
  ...
  if (needsConfirm) {
    ...
    if (mismatch) {
      reply_(renderTenantKey_("CONFIRM_CONTEXT_MISMATCH", lang, {
        ...baseVars,
        dirPropertyName: String(propertyName || "").trim(),
        dirUnit: String(pendingUnit || "").trim(),
        dbPropertyName: dbName,
        dbUnit: dbUnit
      }));
    } else {
replyNoHeader_(renderTenantKey_("CONFIRM_CONTEXT_NEEDS_CONFIRM", lang, {
  ...baseVars,
  dbPropertyName: dbName,
  dbUnit: dbUnit
}));
...
if (pendingStage === "CONFIRM_CONTEXT") {
  ...
  if (yes) {
    ...
    if (nextMissing === "READY") {
      ...
      if (!result.ok) {
        if (result.reason === "ACTIVE_TICKET_EXISTS") {
          ...
          reply_(renderTenantKey_("ERR_DRAFT_FINALIZE_FAILED", lang, baseVars));
          return;
        }
        ...
        if (result.nextStage === "") {
          if (!result.ackOwnedByPolicy) {
            replyNoHeader_(renderTenantKey_("TICKET_CREATED_COMMON_AREA", lang, Object.assign({}, baseVars, { ticketId: String(result.ticketId || "") })));
            try { if (dirRow > 0 && typeof advanceTenantQueueOrClear_ === "function") advanceTenantQueueOrClear_(sheet, dir, dirRow, phone, lang); } catch (_) {}
          } else {
            try { logDevSms_(phone, "", "ACK_SUPPRESSED_BY_POLICY workItemId=" + (result.createdWi || "") + " rule=" + (result.policyRuleId || "")); } catch (_) {}
          }
          ...
          const dayLine2 = dayWord
            ...
            : "";
          replyNoHeader_(renderTenantKey_("ASK_WINDOW_SIMPLE", lang, Object.assign({}, baseVars, { dayLine: dayLine2 })));
        }
        return;
      }
    }
    ...
    else if (nextMissing === "UNIT") reply_(renderTenantKey_("ASK_UNIT", lang, baseVars));
    else reply_(renderTenantKey_("ASK_ISSUE_GENERIC", lang, baseVars));
    return;
  }
  ...
  reply_(renderTenantKey_("CONFIRM_CONTEXT_YESNO_REPROMPT", lang, {
    ...baseVars,
    dbPropertyName: String(ctxc && ctxc.propertyName ? ctxc.propertyName : "the tenant record"),
    dbUnit: String(ctxc && ctxc.unit ? ctxc.unit : "")
  }));
...
if (pendingStage === "EMERGENCY_DONE") {
  ...
  reply_(renderTenantKey_("EMERGENCY_UPDATE_ACK", lang, baseVars));
  return;
}
...
if (ticketBoundNoRow) {
  ...
  reply_(renderTenantKey_("ASK_ISSUE_GENERIC", lang, baseVars));
...
if (effectiveStage === "SCHEDULE_DRAFT_MULTI") {
  ...
  replyNoHeader_(renderTenantKey_("MULTI_CREATED_CONFIRM", lang, confirmVars));
  return;
}
...
if (effectiveStage === "PROPERTY" || effectiveStage === "UNIT" || effectiveStage === "ISSUE") {
  ...
  if (effectiveStage === "PROPERTY") {
    ...
    try {
      ...
      const propsList = getActiveProperties_();
      if (!propsList || !propsList.length) {
        reply_(renderTenantKey_("ERR_NO_PROPERTIES_CONFIGURED", lang, baseVars));
        return;
      }
...
if (!pObj) {
  ...
  return;
      ...
      if (pendingRow < 2) {
        if (!pendingIssue) {
          reply_(renderTenantKey_("ERR_LOST_OPEN_REQUEST", lang, baseVars));
      ...
      if (recovResult) {
        ...
        if (nextStage === "") {
          if (!(recovResult && recovResult.ackOwnedByPolicy)) {
            replyNoHeader_(renderTenantKey_("TICKET_CREATED_COMMON_AREA", lang, Object.assign({}, baseVars, { ticketId: String((recovResult && recovResult.ticketId) || "") })));
            try { if (dirRow > 0 && typeof advanceTenantQueueOrClear_ === "function") advanceTenantQueueOrClear_(sheet, dir, dirRow, phone, lang); } catch (_) {}
          } else {
          ...
          const dayLineRecov = dayWord
            ? ("\n" + renderTenantKey_("ASK_WINDOW_DAYLINE_HINT", lang, Object.assign({}, baseVars, { dayWord })))
            : "";
          replyNoHeader_(renderTenantKey_("ASK_WINDOW_SIMPLE", lang, Object.assign({}, baseVars, { dayLine: dayLineRecov })));
        }
        return;
      }
      ...
      if (hasTicketPtr && isEmergencyContext_(sheet, pendingRow)) {
        ...
        if (tid) replyNoHeader_(renderTenantKey_("EMERGENCY_CONFIRMED_DISPATCHED_WITH_TID", lang, { ...baseVars, ticketId: tid }));
        else     replyNoHeader_(renderTenantKey_("EMERGENCY_CONFIRMED_DISPATCHED", lang, baseVars));
      ...
      if (isClear) {
        ...
        // ✅ SINGLE schedule prompt (no duplicate, no dayLine var)
        reply_(renderTenantKey_(dayWord ? "ASK_WINDOW_WITH_DAYHINT" : "ASK_WINDOW", lang, { ...baseVars, dayWord }));
      ...
      reply_(renderTenantKey_("ERR_GENERIC_TRY_AGAIN", lang, baseVars));
      return;
    }
  }
...
if (effectiveStage === "UNIT") {
  ...
  if (isClear) {
    ...
    // ✅ template-key only schedule prompt (no dayLine stacking)
    reply_(renderTenantKey_(dayWord ? "ASK_WINDOW_WITH_DAYHINT" : "ASK_WINDOW", lang, { ...baseVars, dayWord }));
...
// ✅ Clear ISSUE expectation on success (prevents loops)
  ...
  if (activeRow < 2) {
    ...
    if (!_dirUnit) {
      reply_(renderTenantKey_("ASK_UNIT", lang, Object.assign({}, baseVars, { propertyName: _dirPropName || _dirPropCode })));
      try {
        ctxUpsert_(phone, {
          pendingWorkItemId: "",
          pendingExpected: "UNIT",
    ...
    if (_recov && _recov.ok && _recov.multiIssuePending) {
      ...
      if (_nextMi === "UNIT") { reply_(renderTenantKey_("ASK_UNIT", lang, baseVars)); return; }
    ...
    if (_recov && _recov.ok && _recov.loggedRow >= 2) {
      ...
      if (_nextSt === "") {
        if (!_recov.ackOwnedByPolicy) {
          replyNoHeader_(renderTenantKey_("TICKET_CREATED_COMMON_AREA", lang, Object.assign({}, baseVars, { ticketId: String(_recov.ticketId || "") })));
          try { if (dirRow > 0 && typeof advanceTenantQueueOrClear_ === "function") advanceTenantQueueOrClear_(sheet, dir, dirRow, phone, lang); } catch (_) {}
        } else {
      ...
      if (_nextSt === "UNIT") { reply_(renderTenantKey_("ASK_UNIT", lang, baseVars)); return; }
      if (_nextSt === "SCHEDULE") {
        replyNoHeader_(renderTenantKey_("ASK_WINDOW_SIMPLE", lang, baseVars));
        return;
      }
  ...
  if (isClear) {
    ...
    // ✅ template-key only schedule prompt (no tenantMsg_, no dayLine)
    reply_(renderTenantKey_(dayWord ? "ASK_WINDOW_WITH_DAYHINT" : "ASK_WINDOW", lang, { ...baseVars, dayWord }));
...
function handleScheduleMultiBuffer_(sheet, dir, dirRow, phone, rawTrim, lang, baseVars, stageDay, TWILIO_SID, TWILIO_TOKEN, TWILIO_NUMBER, ONCALL_NUMBER) {
  ...
  if (!verdict.ok) {
    replyNoHeader_(renderTenantKey_(verdict.key, lang, Object.assign({}, baseVars, verdict.vars || {})));
  ...
  var confirmMsg = (typeof renderTenantKey_ === "function")
    ? renderTenantKey_("VISIT_CONFIRM_MULTI", lang, Object.assign({}, baseVars, { visitId: visitId, ticketIds: ticketIds.join(", "), label: label }))
    : ("Visit " + visitId + " scheduled for " + label + ". Tickets: " + ticketIds.join(", "));
  replyNoHeader_(confirmMsg);
  try { logDevSms_(phone, confirmMsg.slice(0, 80), "VISIT_CONFIRM_SENT"); } catch (_) {}
  try {
...
function handleScheduleSingleTicket_(sheet, dir, dirRow, phone, activeRow, rawTrim, lang, baseVars, dayWord, turnFacts, sidSafe, now, signals) {
  ...
  const label = windowLabel_(rawTrim, stageDay);
  if (!label) {
    replyNoHeader_(renderTenantKey_(dayWord ? "ASK_WINDOW_WITH_DAYHINT" : "ASK_WINDOW", lang, Object.assign({}, baseVars, { dayWord: dayWord || "" })));
    return true;
  }
  ...
  // ✅ template-only confirmation message (tone-based key)
  ...
  try { logDevSms_(phone, outMsg, "DEBUG_CONF_WINDOW_SET_RENDER"); } catch (_) {}
  replyNoHeader_(outMsg);
...
if (String(effectiveStage || pendingStage || "").toUpperCase() === "SCHEDULE") {
  ...
  // Helper: re-ask schedule + keep ctx expectation
  function reAskSchedule_(reason) {
    replyNoHeader_(renderAskWindow_("ASK_WINDOW_SIMPLE"));
...
if (effectiveStage === "FINALIZE_DRAFT") {
  ...
  if (result.nextStage === "EMERGENCY_DONE") {
    ...
    replyNoHeader_(renderTenantKey_("EMERGENCY_TENANT_ACK", lang, Object.assign({}, baseVars, { ticketId: eTid })));
    return;
  }
  ...
  replyNoHeader_(renderTenantKey_("ASK_WINDOW_AFTER_CREATE", lang, vars));
  return;
}
...
if (ticketState.stateType === "NEW") {
  ...
  if (domainDispatchResult && domainDispatchResult.handled === true) {
    if (typeof clearMaintenanceDraftResidue_ === "function") clearMaintenanceDraftResidue_(dir, dirRow, phone);
    try {
      replyNoHeader_(renderTenantKey_("CLEANING_WORKITEM_ACK", lang, baseVars));
    } catch (_) {}
    return;
  }
  ...
  if (!looksActionableIssue_(rawTrim)) {
    replyNoHeader_(renderTenantKey_("PROMPT_SEND_MAINT_FORMAT", lang, baseVars));
    return;
  }

  // â”€â”€ B) Clear stale draft fields before starting fresh â”€â”€
  if (dirRow > 0) {
  ...
  if (nextMissing !== "READY") {
    ...
    else /* ISSUE */                     reply_(renderTenantKey_("ASK_ISSUE_GENERIC", lang, baseVars));
    return;
  }
  ...
  if (cache.get(dkey)) {
    reply_(renderTenantKey_("DUPLICATE_REQUEST_ACK", lang, baseVars));
    return;
  }
  ...
  if (nextStage === "") {
    if (!result.ackOwnedByPolicy) {
      replyNoHeader_(renderTenantKey_("TICKET_CREATED_COMMON_AREA", lang, Object.assign({}, baseVars, { ticketId: String(ticketId || "") })));
      try { if (dirRow > 0 && typeof advanceTenantQueueOrClear_ === "function") advanceTenantQueueOrClear_(sheet, dir, dirRow, phone, lang); } catch (_) {}
    } else {
  ...
  if (String(mode || "").toUpperCase() === "MANAGER") {
    ...
    replyNoHeader_(String(intro || "").trim() + "\n\n" + String(ask || "").trim());
    return;
  }

  replyNoHeader_(renderTenantKey_("ASK_WINDOW_SIMPLE", lang, Object.assign({}, baseVars, { dayLine })));
  return;
...
try {
  ...
  if (pr2 > 1) {
    ...
    if (exp2 === "SCHEDULE") {
      reply_(renderTenantKey_("ASK_WINDOW", lang, { ...baseVars, dayLine: dayLine2 }));
      try {
        ctxUpsert_(phone, {
          pendingExpected: "SCHEDULE",
          pendingExpiresAt: new Date(Date.now() + 30 * 60 * 1000),
    ...
    // Unknown but active ticket → safest schedule prompt, canonical expectation
    reply_(renderTenantKey_("ASK_WINDOW", lang, { ...baseVars, dayLine: dayLine2 }));
    try { ctxUpsert_(phone, { pendingExpected: "SCHEDULE", pendingExpiresAt: new Date(Date.now() + 30 * 60 * 1000), lastIntent: "MAINT" }); } catch (_) {}
    return;
...
function sendRouterSms_(to, text, tag) {
  const msg = String(text || "").trim();
