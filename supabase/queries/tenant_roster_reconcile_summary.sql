-- STEP 1 — Paste this result in chat for review (compact).
-- Supabase SQL Editor → Run → copy results table.
...
norm_units as (
  select
    ...
    upper(trim(regexp_replace(trim(u.unit_label),
      '^(apt|apartment|departmento|apartamento|suite|ste|rm|room|unit|u)\.?\s*[:#-]?\s*', '', 'i'))) as unit_norm
  from public.units u
),
roster_norm as (
  select
    tr.id as roster_id, tr.active, tr.updated_at,
...
-- A) Portfolio summary — paste this block first
select status, count(*) as units
from final
group by 1
order by units desc;

-- B) Issues only — comment A, uncomment B for detail paste
/*
select property_code, unit_label, status,
  lh_tenant_name, primary_roster_name,
  lh_phone_raw, primary_roster_phone, active_summary
from final
where status not in ('match', 'both_vacant')
order by property_code, status, unit_label;
*/
