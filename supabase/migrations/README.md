...
| **019_properties_program_expansion_profile.sql** | `properties.program_expansion_profile` (jsonb) — per-property PM expansion hints | **`expandProgramLines.js`**: `FLOOR_BASED` uses **`floor_paint_scopes`** then **`common_paint_scopes`**; `COMMON_AREA_ONLY` uses **`common_paint_scopes`** only — see **`docs/PM_PROGRAM_ENGINE_V1.md`** |
